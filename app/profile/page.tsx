...
export default function ProfilePage() {
      ...
      try {
        ...
        if (userEmail) {
          ...
          if (fetchedUser) {
            ...
            const stats = fetchedTransactions.reduce((acc: any, transaction) => {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      {/* Stats Grid */}
      <h2 className="text-2xl font-semibold mb-4 text-gray-800">Activity Summary</h2>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <StatCard icon={MapPin} title="Reports Submitted" value={stats.reportsSubmitted} />
        <StatCard icon={Trash} title="Waste Collected" value={stats.wasteCollected} />
        <StatCard icon={Coins} title="Points Earned" value={stats.pointsEarned} />
        <StatCard icon={Coins} title="Points Redeemed" value={stats.pointsRedeemed} />
      </div>
...
function StatCard({ icon: Icon, title, value }: { icon: React.ElementType; title: string; value: number }) {
  return (
