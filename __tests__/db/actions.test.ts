...
jest.mock('../../utils/db/dbConfig', () => ({
  db: {
    insert: jest.fn(() => ({
      values: jest.fn(() => ({
        returning: jest.fn(() => ({
          execute: jest.fn(() => [{ id: 1, email: 'test@example.com', name: 'Test User' }])
        }))
      }))
    })),
    select: jest.fn(() => ({
      from: jest.fn(() => ({
        where: jest.fn(() => ({
          ...
          orderBy: jest.fn(() => ({
            limit: jest.fn(() => ({
              execute: jest.fn(() => [
                { id: 1, type: 'earned_report', amount: 10, description: 'Test transaction', date: new Date() }
              ])
            }))
          }))
        })),
...
describe('Database Actions', () => {
  ...
  describe('getUserBalance', () => {
    test('should never return negative balance', async () => {
      ...
      const mockTransactions = [
        { id: 1, type: 'earned_report', amount: 10, description: 'Test', date: '2024-01-01' },
        { id: 2, type: 'redeemed', amount: 20, description: 'Test', date: '2024-01-02' }
      ];
  ...
  describe('createTransaction', () => {
    test('should create transaction successfully', async () => {
      ...
      const result = await dbActions.createTransaction(userId, type, amount, description);
